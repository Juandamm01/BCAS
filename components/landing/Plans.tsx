...
const formatZoneLabel = (zone: string) => {
  ...
  if (key.includes("mesetas")) return ZONE_LABELS.mesetas;
  if (key.includes("nohora") || key.includes("nohora y san luis")) return ZONE_LABELS.nohora;
  if (key.includes("zuria")) return ZONE_LABELS.zuria;
...
const Plans = ({ initialPlans }: PlansProps) => {
  const plans = useMemo(() => initialPlans || [], [initialPlans]);
  const initialTab = plans[0]?.zona || "";
    ...
    <section id="plans" className="py-24 bg-black relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col mb-12">
          ...
          <div className="flex gap-3 border-b border-white/10 overflow-x-auto no-scrollbar pb-4">
            {zones.map((zone) => (
              <button
                key={zone.id}
                onClick={() => setActiveTab(zone.id)}
                className={cn(
                  "px-4 py-2.5 rounded-xl text-sm md:text-base font-semibold transition-all relative whitespace-nowrap cursor-pointer flex-shrink-0 border",
                  activeTab === zone.id
                    ? "text-white border-white/40 bg-white/5"
                    : "text-slate-400 border-white/10 hover:text-slate-200 hover:border-white/30"
                )}
              >
                {zone.label}
                {activeTab === zone.id && (
                  <motion.div layoutId="tab-indicator" className="absolute -bottom-[17px] left-3 right-3 h-0.5 bg-white" />
                )}
              </button>
            ))}
          </div>
                ...
                >
                  ...
                  <div className="mt-5 border-t border-white/10 relative pt-5">
                    ...
                    <div className="flex items-center text-slate-400 text-sm">
                      ...
                      <span>{plan.tv || "1 Punto de TV GRATIS"}</span>
                    </div>
                  </div>
                </div>
              ))}
